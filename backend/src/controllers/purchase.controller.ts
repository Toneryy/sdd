...
function genOrderNumber(len = 7): string {
  ...
  return s; // ведущие нули сохраняются как строка
}
...
export const devCheckout: RequestHandler = async (req, res) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      // 1) создаём заказ с рандомным order_number + ретраи
      let order;
      for (let attempt = 1; attempt <= 10; attempt++) {
        const candidate = genOrderNumber(7); // всегда 7
        try {
          ...
          // коллизия — пробуем ещё раз
        }
      }

      if (!order) throw new Error("ORDER_NUMBER_GEN_FAILED");

      // 2) дальше как было: добавляем позиции, резервируем ключи
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const it of order.items) {
        if (it.item_type === "product" && it.product_id) {
          // заберём ключи, зарезервированные этим заказом для этого продукта
          ...
          for (const pk of reserved) {
            ...
            // генерим alias с ретраями на коллизию
            let aliasCode = "";
            let aliasId = "";
            for (;;) {
            ...
            // прикрепим alias к одному свободному unit этой строки
...
// --- DEV: ACTIVATE (только для продуктов) ---
export const devActivate: RequestHandler = async (req, res) => {
  ...
  try {
    const out = await prisma.$transaction(async (tx) => {
      // Берём alias + проверяем, что он принадлежит заказу этого пользователя
      const rows = await tx.$queryRaw<
        Array<{ id: string; product_key_id: string; order_user_id: string }>
      >`
        SELECT ka.id, ka.product_key_id, o.user_id as order_user_id
      ...
      if (alias.order_user_id !== userId) {
        throw new Error("FORBIDDEN_ALIAS"); // код не ваш
      }
